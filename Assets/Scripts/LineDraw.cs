...
public class LineDraw : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if ( Input.GetMouseButton( 0 ) )
        {
            ...
            if( Vector2.Distance( finger_pos, line_points[line_points.Count - 1 ]) > 1f)
            {
        ...
        if( Input.GetMouseButtonUp( 0 ) )
        {
            ...
            for (int i = 1; i < line_points.Count; i++)
            {
                ...
                {
                    up = halfWidth * new Vector2(crossProduct.normalized.x, crossProduct.normalized.y) + line_points[ i ];
